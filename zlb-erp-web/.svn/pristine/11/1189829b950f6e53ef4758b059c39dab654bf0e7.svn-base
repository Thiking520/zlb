...
var supplierManager = {
	...
	URL : {
		...
		rejectSupplierUrl : function() {
			return '/supplier/rejectSupplier';
		}
	},
	/** 分页获取商品列表* */
	...
	showDetail : function(supplierId, supplierState) {
		...
		$.callAjax({
			...
			success : function(data) {
				...
				$('#license').text(data.data.license);
				$('#organization').text(data.data.organization);
				$('#contractCopy').text(data.data.contractCopy);
	...
	bindEvent : function() {
    	...
    	$("#btn_tiao_goodsList").click(function(){
    		myMain.getAllContent(supplierManager.URL.initUrl());
    	});
	},
