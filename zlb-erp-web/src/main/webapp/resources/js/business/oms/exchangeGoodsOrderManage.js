...
var exchangeGoodsOrderManage = {
    ...
    URL: {
    	...
    	searchListByPageUrl: function () {
            return '/orderManage/exchangeGoodsOrder/list';
        },//更新换货订单备注的请求地址
        updateRemarkUrl: function () {
            return '/orderManage/exchangeGoodsOrder/updateRemark';
        },//批量导出换货订单详情
        exportExchangeGoodsOrderDetailListUrl: function () {
        	return '/orderManage/exchangeGoodsOrder/exportExcel';
        },//批量审核换货订单详情
        batchAuditUrl: function () {
        	return '/orderManage/exchangeGoodsOrder/batchAudit';
	    },//批量打印换货订单详情
        batchPrintUrl: function () {
        	return '/orderManage/exchangeGoodsOrder/batchPrint';
	    },//初始化下拉框数据请求地址
	    initDropDownBoxUrl: function () {
	    	return '/orderManage/exchangeGoodsOrder/initDropDownBox';
	    },
    	//查询订单详情地址
    ...
    searchListByPage: function () {
    	...
    	//分页组件
    	$.pageTable({
    		tableId: "#exchangeGoodsOrderManageTable",//需要分页的table ID
    		url: exchangeGoodsOrderManage.URL.searchListByPageUrl(),//请求后台的URL（*）
    		method: 'POST',//默认启用POST
    		queryParams:queryParams,
    		onLoadSuccess:function(){
    			exchangeGoodsOrderManage.isResetOffset = 0;
    			$("#btn_search").removeClass("disabled");
                $.resizeOrderTable("exchangeGoodsOrderManageTable");
            },
            sortable: true,
    		sortName:'sale_time',
    		sortOrder:'desc',
...
//    	        	var lockBtn = '<a class="btn btn-info lockOrder_a" href="javascript:void(0)" >订单锁定</a>';
    	        	...
    	        	var html='';
    				var updateRemarkUrl = exchangeGoodsOrderManage.URL.updateRemarkUrl();
...
//    	        	}
    ...
    getOrderDetails: function (id) {
    	...
    	var url = exchangeGoodsOrderManage.URL.selectExchangeOrdersDetails()+'?id='+id+"&params="+params;
    ...
    exportOrderDetails: function () {
    	...
    	//触发Ajax
    	$('#exchangeGoodsOrderManageTable').bootstrapTable('refresh');
    	var params = exchangeGoodsOrderManage.globalParams;
//    	var params = $('#exchangeGoodsOrderManageTable').bootstrapTable('getOptions').queryParams();
    	
    	$.callAjax({
    		url : exchangeGoodsOrderManage.URL.exportExchangeGoodsOrderDetailListUrl(),
    		data : params,
    		success : function(data){
    			if(data.code != "0000" || $.isNull(data.data)){
    				...
    				if($.isNotNull(data.data)){
    					...
    					location.href=contextPath+"/orderManage/exchangeGoodsOrder/download?fileName="+fileName;
    				}else{
...
//    	params = JSON.stringify(params);
    	$.callAjax({
    		type:"post",
    		url : exchangeGoodsOrderManage.URL.updateRemarkUrl(),
    		data : params,
    		success : function(data){
    			if(data.code != "0000"){
    				$.toastrWarning(data.msg);
...
//    	params = JSON.stringify(params);
    	$.callAjax({
    		type:"post",
    		url : exchangeGoodsOrderManage.URL.batchAuditUrl(),
    		data : params,
    		success : function(data){
    			if(data.code != "0000"){
    				$.toastrWarning(data.msg);
    ...
    //批量打印
    batchPrint: function (orderIds) {
    	//触发Ajax
    	var params = "?ids="+orderIds;
		var contextPath = $("#contextPath").val();
		//请求下载excel
		window.open(contextPath + exchangeGoodsOrderManage.URL.batchPrintUrl() + params);
...
//    	$.callAjax({
//    		type:"post",
//    		url : exchangeGoodsOrderManage.URL.batchPrintUrl(),
//    		data : params,
//    		success : function(data){
//    			if(data.code != "0000"){
//    				$.toastrWarning(data.msg);
...
//    				//组织打印数据
//    				var printContent = "";
//    				var model = $("#printModel").html();
//    				for(var i=0;i<100;i++){
//    					$("#printContent").append(model);
//    				}
//    				printContent = $("#printContent").html();
//    				exchangeGoodsOrderManage.jsprint(printContent);
//    				$.toastrSuccess('操作成功！');
//    			}
//    		},
//    		error : function(){
...
//    },
    //允许/拒绝弹出框
    batchAuditDialog:function(message,label1,label2,orderIds,orderState){
    	//操作1允许,2拒绝
    	BootstrapDialog.show({
            ...
            buttons: [{
                ...
                action: function(dialogRef){
                    ...
                    exchangeGoodsOrderManage.batchAudit(orderIds,orderState,operation);
                    dialogRef.close();
                }
            }, {
    ...
    showServiceBtn:function(){
		...
		var btn_show_export = exchangeGoodsOrderManage.URL.exportExchangeGoodsOrderDetailListUrl();
		var btn_show_batch_audit = exchangeGoodsOrderManage.URL.batchAuditUrl();
    ...
    bindEvent: function () {
    	...
    	$(".orderStateClass").click(function(){
    		...
    		//展示批量审核，批量打印，批量导出等按钮
    		exchangeGoodsOrderManage.showServiceBtn();
    		$('#exchangeGoodsOrderManageTable').bootstrapTable('refresh');
    	});
    	...
    	$("#btn_search").on("click",function () {
    		...
    		exchangeGoodsOrderManage.isResetOffset = 1;
    		$('#exchangeGoodsOrderManageTable').bootstrapTable('refresh');
    	});
        //绑定展示显示搜索条件按钮事件
...
//		     $.showModal('#myModal');;
    		exchangeGoodsOrderManage.exportOrderDetails();
	    });
    	//绑定展示批量打印界面事件
    	$("#btn_show_print").click(function () {
    		var orderIds = $.getIdSelections("#exchangeGoodsOrderManageTable","id");
        	if(orderIds==null||orderIds==''){
        		$.toastrWarning('请先选择记录再操作！');  
        		return false;
        	}
    		exchangeGoodsOrderManage.batchPrint(orderIds);
    	});
    	//绑定展示批量审核界面事件
...
//    			0.已取消
    			...
    			if(orderState == '1'){
    				...
    				exchangeGoodsOrderManage.batchAuditDialog(message,'同意换货','拒绝换货',orderIds,orderState);   				
    			}
    			//待配送员揽收
    			if(orderState == '5'){
    				var message = '请选择你要进行的操作！';
    				exchangeGoodsOrderManage.batchAuditDialog(message,'同意揽收','拒绝揽收',orderIds,orderState);   				
    			}
    			//待用户签收
    			if(orderState == '3'){
    				var message = '请选择你要进行的操作！';
    				exchangeGoodsOrderManage.batchAuditDialog(message,'确认签收','拒绝签收',orderIds,orderState);   				
    			}
    		}
    	});
    	//绑定展示新增界面事件
    	...
    	$("#btn_remark_save_submit").on("click",function () {  
	    	...
	    	//触发更新换货订单备注信息的方法
	    	exchangeGoodsOrderManage.updateRemark(id,distributionNote);
    	});
    },//初始化分页查询列表数据 ★★★分页主体列表★★★
    init: function () {
...
//        $(".form_datetime").datetimepicker({
        ...
        exchangeGoodsOrderManage.showServiceBtn();
        //初始化下拉框
        exchangeGoodsOrderManage.initDropDownBox();
    	exchangeGoodsOrderManage.searchListByPage();
    },
    //初始化下拉框数据
...
//    	var params = {};
    	$.callAjax({
			type:"post",
			url : exchangeGoodsOrderManage.URL.initDropDownBoxUrl(),
//			data : params,
			success : function(data){
         		if(data.code != "0000"){
         			$.toastrWarning(data.msg);
         		...
         		for(var i=0;i<queryList.length;i++){
         			if(queryList[i].dictValue == '2' || queryList[i].dictValue == '4' || queryList[i].dictValue == '8' || queryList[i].dictValue == '10'){
         				continue;
         			}
...
//         			if(subsetTypeList[i].dictValue == '2' || subsetTypeList[i].dictValue == '4' || subsetTypeList[i].dictValue == '5')
         			if(subsetTypeList[i].dictValue == '0' || subsetTypeList[i].dictValue == '4' || subsetTypeList[i].dictValue == '5')
...
var queryParams = function (params) {
	...
	var temp = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
		...
		so:{
			...
			offset: exchangeGoodsOrderManage.isResetOffset == 1 ? 0 : params.offset,  //分页偏移值
	...
	exchangeGoodsOrderManage.globalParams = temp;
...
//	    	$.toastrSuccess('换货订单编号为： ' + row.id);
	    	//触发查询换货订单详情的方法
	    	exchangeGoodsOrderManage.getOrderDetails(row.id);
	    },
